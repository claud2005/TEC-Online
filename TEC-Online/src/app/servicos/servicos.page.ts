...
export class ServicosPage implements OnInit {
  ...
  formatarData(data: string): string {
    ...
    return data.includes('T') ? `${day}-${month}-${year} ${hours}:${minutes}` : `${day}-${month}-${year} 00:00`;
  }
    ...
    this.http.get<Servico[]>(`${environment.api_url}/api/servicos`, { headers }).subscribe(
      (response) => {
        this.servicos = response.map((servico, index) => ({
          ...
          dataServico: this.formatarData(servico.dataServico),
        }));
  ...
  filtrarServicos() {
    ...
    this.servicosFiltrados = this.servicos.filter(servico => {
      ...
      const matchSearch =
        ...
        servico.numero?.toString().toLowerCase().includes(termo);
  ...
  async gerarPDF(servico: Servico) {
    ...
    const section = (title: string) => {
      doc.setFont('helvetica', 'bold');
      doc.setTextColor(primaryColor);
      doc.text(title, leftMargin, verticalPos);
  ...
  async getBase64ImageFromAssets(imagePath: string): Promise<string> {
    ...
    return new Promise<string>((resolve, reject) => {
      ...
      reader.onloadend = () => {
        typeof reader.result === 'string' ? resolve(reader.result) : reject('Erro ao converter imagem para base64');
      };
